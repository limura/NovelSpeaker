...
class SpeechViewController: UIViewController, StorySpeakerDeletgate, RealmObserverResetDelegate, UIGestureRecognizerDelegate, UITextViewDelegate {
    ...
    var bottomVoicevoxCacheGenerationButtonItem:UIButton? = nil
    ...
    func assignBottomButtons() {
        DispatchQueue.main.async {
            ...
            self.bottomVoicevoxCacheGenerationButtonItem = nil
    ...
    func createSpeechViewButtonArray(novelID: String, novelType:NovelType, aliveButtonSettings:[SpeechViewButtonSetting], buttonSize:CGFloat, isForBottomBar:Bool) -> [UIButton] {
        ...
        for buttonSetting in aliveButtonSettings {
            ...
            case .voicevoxCacheGeneration:
                ...
                if isForBottomBar {
                    self.bottomVoicevoxCacheGenerationButtonItem = button
                } else {
    ...
    @objc func voicevoxCacheProgressDidChange() {
        DispatchQueue.main.async { [weak self] in
            ...
            let novelID = RealmStoryBulk.StoryIDToNovelID(storyID: self.storyID ?? "")
            let isGenerating = VoicevoxCacheGenerator.shared.runningNovelID == novelID
            let image = UIImage(systemName: isGenerating ? "v.circle.fill" : "v.circle")
            // 上のボタン群と下のボタンバーの両方に出ている事があるので、どちらも直す。
            self.voicevoxCacheGenerationButtonItem?.setImage(image, for: .normal)
            self.bottomVoicevoxCacheGenerationButtonItem?.setImage(image, for: .normal)
        }
    }

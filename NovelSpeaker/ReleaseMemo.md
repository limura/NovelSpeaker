...
・なろう検索のタブで、検索時の順位付けを「総合評価の高い順」などから選べるようにしました
...
追伸：
レビューありがとうございます。早めに対応できそうな不都合の報告がありましたので、主に不都合対応のリリースをさせていただきます。
その他、提案していただいた機能で、簡単に追加できそうなものについては追加してみました。ご確認ください。

本棚周りはフォルダにするであるとか並び順を変えるであるとかいった事をまとめてやろうと思って書き始めた所、結構時間がかかりそうな事がわかりましたので、今回のリリースには含めることができませんでした。すみません。なお、これの対応については結構かなりいっぱいたくさん時間がかかってしまいそうですので、気長に待っていただければ嬉しいです。

コピー＆ペーストなどで小説家になろう以外の文書を読み上げる機能ですが、これも結構時間がかかりそうです。というのも、これは本当に自分のミスなのですが、小説家になろう以外の小説データが入る事を全く考えずにデータベースを構築してしまったのでそのあたりを書き換えないと駄目なので、これも結構大変なのです。すみません。また、青空文庫やその他の小説を公開してくれているサイトへの対応をするとなると、微妙な違いがある(例えばルビの扱いとかが小説家になろうと違うものが結構あるようですし、小説の検索用のAPIが用意されていないものが多く、これも対応が面倒くさそうです)のでそのあたりの対応等に時間がかかりそうな予感がしています。なので、手っ取り早くはコピペ位かなと思っていますので、まずはコピペをサポートする所から手をつけようと思っています。
あと、青空文庫でしたら(自分のアプリではないですが)Umemoto Nonさんの「音声文庫」というアプリがありますのでそちらを使うというのでもいいかもわかりません。

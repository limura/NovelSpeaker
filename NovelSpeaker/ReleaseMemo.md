...
- Safariで利用できるシェアボタン(上向きの矢印のボタン)から利用できる「ことせかいへ読み込む」機能を追加しました。Safariで "小説の本文が表示されている状態"(小説のタイトルページではなくて本文が表示されているページです) で、シェアボタン(四角から上向きの矢印が出ているボタン)を押して、「ことせかいへ読み込む」を選択すると利用できます。(標準ではOFFになっていて表示されませんので、シェアボタンを押した後、「その他」の中で「ことせかいへ読み込む」をONにしてからご利用ください)
- イヤホンジャックにイヤホンが刺さっている時にイヤホンが引き抜かれた場合、再生が停止するようになります

追伸

評価やレビュー、ありがとうございます。特にサポートサイト側からのご意見ご要望フォームからの詳細なバグ報告には本当に助かっています。これからも宜しくお願い致します。
今回は棚上げになっていた小説家になろう以外のサイトについてへの対応について、こんな感じではどうだろう、というものを実装してみました。EPUB等の読み込みも考えてはいるのですが、まだまだEPUBで直接出回っているような書籍があまりなく、WebページをEPUBに変換して読み込む、などの用途になってしまって結局手間がかかってしまいそうだということから、それならばWebページから直接読み込めるようにしたほうが利便性は高いだろうという目論見で実装してみました。ただ、世の中には色々な形式のWebページがあり、その全てを読み込めるようにするのは大変なので、Webページを取り込む時の補佐、程度の性能となっております。複数ページがある場合には続きのページも出来る限り読み込むようにはしましたが、全てのWebページで動作するようには作られておりませんので、あまり期待せずにうまく動いたらラッキー、位で思っておいていただけると嬉しいです(実際、いくつかのWebサイトでは動かないのを確認しています。その場合はお手数ですが今まで通りコピーして自作小説としてペーストするなどで対応していただければ嬉しいです。なお、サポートサイト側などから「このサイトがうまく動きませんでした」と報告していただけると嬉しいですが、すぐには対応できないかもしれません)。また、読み込まれた文章に本文と関係の無いもの(例えば「次のページへ」のリンクの文字など)が混じることがあります。そのため、これらを修正できるように、Webページから読み込まれた文章は自作小説と同じ扱いをしていますので、編集して取り除いて頂ければと思います。
それから、再度のご案内となりますが、次回以降の更新時に、ことせかい の対応OSバージョンを iOS 8 以上にすることを計画しています(現在は iOS 7 以上の対応です)。どうしても iOS 7 で動作して欲しいという方がおられましたら、サポートサイトの下の方に用意致しましたご意見ご要望フォームから思いの丈を送っていただけると対応できるようになるかもしれません。

...
追伸
評価やレビュー、ありがとうございます。長いことおまたせしてしまって申し訳ありません。自作小説の登録機能を追加してみました。ただ、この機能は内部的にちょっと汚い形で実装してしまったので、書き直しを検討しています。
具体的には、電子書籍のEPUB形式を検討しています。というのも、EPUB形式を調べてみると、生成や読み込みに関しては案外簡単にできそうな気がしましたので、内部での保存形式をEPUBにするような変更をしようかな(そうすると今まで生テキスト形式で保存してたのがZIPで圧縮されて嬉しいし、小説家になろう以外でEPUBで公開してくれているサイトのものも取り込みやすくなるしいいかもしれないなぁ、ああでも内部形式がかなり変更されるのでやるとすると大変だぞ？)と思っています。ちょっと大変そうなのでまた長いことお待たせる可能性が非常に高いですが、EPUB対応がされると嬉しい人が多かったりすると、やる気が増すと思いますのでレビュー等で叱咤激励していただければと思います。

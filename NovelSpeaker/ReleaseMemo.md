...
また、初期に設定する読み替え辞書は更新したのですが、既に使用中の方の読み替え辞書を更新しようとすると、重複したり書き換わったりしてしまう可能性があるのでデータの更新の際には特に新しい辞書を追加するなどの事はしておりません。なので、お手数ですが一旦アンインストールしてから再インストールする(ただし小説のダウンロードからやり直しになってしまいます)か、読み替え辞書を自分でコツコツと書き足していってください。
ユーザさんの読み替え辞書をサーバにアップロードして云々、という事についてなのですが、サーバの管理が発生する(24時間365日起動し続けているサーバを管理するのはちょっと大変な)のと(無いとは思いますが)意図しない読み替え(例えば「あ」を「くぁｗせｄｒｆｔｇｙふじこ」に読み替えるとか)を行う読み替え辞書の登録がなされた場合の排除方法などを考えるのが大変そうだなぁ、と思ったのでとりあえず良い案が浮かぶまでは実装はしないことにしました。

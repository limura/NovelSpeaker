...
- 読み替え辞書
  - 正規表現のものを読み上げ時に注入する
  - 正規表現であるか否かを設定UI側で指定できるように
  - 正規表現であるか否かをリスト表示部で確認できるようにする？(表示できない文字を排除はしないと駄目ぽ？)
  - バックアップの出力と入力で正規表現か否かを指定・取り込みできるように(テストがまだ)
  - InsertDefaultSpeechModConfig と似た感じで正規表現のものを追加する？
  - \x07 の位置を文字の末尾にする(sort向け)

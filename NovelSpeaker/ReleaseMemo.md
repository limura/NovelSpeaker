...
今回もお問い合わせについての回答をしておきます。
なお、ここに回答が書かれるということは、実装しないと決めた事などのものが多くなります(実装するのであれば実装して上部の更新情報に乗せます)ので、基本的にはユーザ様方には寂しい話となりますので心してお読みください。

まず一つ目は、読み替え辞書をクラウド的なものでユーザ間で共有して編集できないかというご提案です。
これは実現したらとても良いものになるとは思うのですが、そのクラウド的な物を維持するのにはお金がかかるので、ことせかい の開発者側としては対応できない案件となります。何らかの形で定常的にお金を生むような形にしていればよかったのかもしれませんが、どう控えめに考えても今から何らかのお金を生む要素を入れると非難轟々になりますよね？なので、やる気はありません。ただ、数年間分のサーバ費用にオマケもつけるからやってくれという人が現れたり、自分がサーバ側を管理運営するのでアプリ側はなんとかしてくれという人が現れるとしたらあるいは、という気もしなくもありませんけれども。(もちろん、その費用が尽きたりサーバ運営をしてくれている人が諦めたらそこでそのクラウド読み替え辞書共有サービスは終了になりますので、不安定なサービスであるといえます)

二つ目は何らかの問題で期待しないデータがダウンロードされてしまった場合の話です。
これは、例えば小説ではなく、「アクセス数が多いのでエラー」といったものがダウンロードされてしまった、というような場合の事です。
現状では、既にダウンロードして登録されてしまった部分を再ダウンロードする方法はありません。
細かい仕様としましては、なろう検索側からダウンロードするものと、Web取込機能でダウンロードするものではダウンロードの仕組みが違いますので一概に同じ動作をするとは言えないのですが、基本的には、Webサーバからダウンロードしようとした時のステータスコードが正常であった場合(200 OKといったものの場合)にはダウンロードが成功したとみなしています。従って、Webサーバ側が「アクセス数が多いのでエラー」といった情報を HTTP 200 OK というステータスコードで返した場合には、それが文章の中身であるとしてダウンロードが成功します。
また、

二つ目はアプリ内からの不都合報告フォームやお問い合わせについて。
アプリ内の設定タブから手繰れる不都合報告フォームでは、不都合の発生日時を入力することを必須にしています。これは、アプリの操作ログを送って頂いても不都合が起こった日時が確定しないとその操作ログのどの部分で問題が発生しているかを判断しにくくなるためです。ただ、残念ながら不都合報告をしてくる人で、この不都合の発生日時をきちんと入れている人はほとんどおられません。正直な話を申しますと、不都合報告をされるユーザ様の多くが、「使えない」不都合報告をされております。「使えない」というのは、不都合報告の情報量が少なすぎて開発者の側で同じ問題を再現できない場合です。再現ができないと、「そのような問題が起きているらしいことはわかったが、原因を特定できない」という状態になり、結果的にはその問題に対して対策が取れなくなります。この問題を少しでも回避するための方策としてのアプリの操作ログであり、その操作ログ上の不都合の発生日時なのです。ですので、不都合の発生日時は必ず入力して頂きたいです。そのため、入力を必須とし、初期値を24時間後にすることでそのままほったらかして報告をしようとすると「未来の日付なので駄目です」と弾かれるようにしています。また、恐らくは不都合が発生した時に不都合報告を書いているのでしょうから、日時を合わせやすいように(発生した瞬間に不都合報告フォームを開いたならば日付を1日戻す操作だけで目的の時間(つまり今)になるように)、24時間後に設定されているのです。以上の理由から、アプリ不都合報告フォームでの不都合の発生日時が24時間後にされていて、値を書き換えないといけないのは意図的です。
蛇足的に申し上げておきますと、アプリの不都合報告については開発者側の善意で対応している案件になります。その善意を踏みにじるような尊大な書き方は開発者の何かを盛大に削っていきますのでお手柔らかにお願いします。尊大なお問い合わせの問題以外にも、最近はお問い合わせの件数が増えてしまって困っています。開発者としましては、ユーザサポートをしたくてアプリを作っているわけではありませんので、お問い合わせ窓口を全部締める(最悪、アプリの公開をやめる)ことも考えていますが、自分がそうされたら寂しいだろうなぁという事や、たまに届くお問い合わせ窓口への激励の投稿などにはとても元気をいただいていますので、全てを閉じてしまうのもなぁと踏みとどまっている形です。
一つ知っておいて頂きたい事としては、少なくとも ことせかい に関しては、開発者はユーザ様には何の見返りも求めていません。実際に金品は何も受け取っていません。それなのに何かを要求してくる形であるところのお問い合わせ窓口を利用するのであれば、それなりの礼儀作法を守り、相手を尊重して頂きたいという事です(別にこういう事は無料のアプリだからという話ではなく、たとえ有料のサービスだったとしても同様に相手を尊重すべきであって〜以下長くなるので略)。

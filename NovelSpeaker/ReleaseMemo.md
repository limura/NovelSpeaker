...
また、お問い合わせ対応に疲れました。お問い合わせが届いた事を告げる音が端末から鳴るのを恐れてしまっていて、心が休まらない感じになっていますし、お問い合わせ対応をするたびに「これは善意の搾取だし、対価が無いのにここまでやるのは不健全である」という事ばかり考えています。そんなわけなのですが、お問い合わせの数を減らす方法でユーザの皆様が納得できる形の方法は何か無いですかね。今の所は月額課金制への移行(ユーザ数を減らす)か、アプリの公開を止める(ユーザ数をゼロにする)、お問い合わせへの対応の一切を止める(不都合が解消しなくなる)、の3つ以外に有効な手段を思いついていません。これらはそれぞれユーザの方々には辛い話にしかならなさそうで、あまり実行する気にはなっていないのですが、私の側としては、何故私はこんな辛い思いをしてまでアプリを公開しているのかわからず、この辛さをどこかに投げ出したくてしょうがない状態になっています。最初は使ってもらえるだけで楽しかったのですが、そう思えなくなってしまっているというのは不幸ですね。やはり健全に稼ぐ形を模索してこなかったのが問題なのかもですが、今から導入するのも手間がかかる上に誰も喜ばないし、八方塞がりな感じです。

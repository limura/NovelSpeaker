...
今回の修正は iOS 16.3 から発生しているメモリリーク問題に対する緩和策となります。

以下に詳しく説明していきます。

まず、今回の設定項目は iOS 16.3 から発生しているメモリリーク問題に対する物になりますので、iOS 16.2 以前の iOS(や iPad OS)で使っている方には必要のない機能になります。

さて、このメモリリーク問題が発生しないようにする方法が確認できました。といっても、読み上げ中の読み上げ位置の情報を受け取らないようにする必要があり、この読み上げ位置が取得できなくなると、発話を停止した時に「今どこを読み上げていたのか」がわからなくなるという問題があるものでした。

少し具体的に書きますと、「メロスは激怒した。必ず、かの邪智暴虐の王を除かなければならぬと決意した。メロスには政治がわからぬ。メロスは、村の牧人である。笛を吹き、羊と遊んで暮して来た。けれども邪悪に対しては、人一倍に敏感であった。」という文章を読み上げさせていて、「メロスは、村の牧人である」という部分を読み上げている時に「Stop」を押して発話を停止させたとします。期待される動作としては「メロスは、村の牧人である」の部分に次の読み上げ開始位置が保存される事が期待されるわけですが、読み上げを停止した時にどの位置を読み上げていたかを受け取れていないために、「メロスは激怒した」の部分、つまり読み上げ開始時に指定していた場所が次の読み上げ開始位置として保存される(というかそもそも読み上げ位置が更新されていない)という事になります。
ただ、会話文などで話者が変わるなどした場合には「その話者での読み上げが終了した」という事がわかるため、話者が変わる位置まで読み上げが進行したという事は確認できるようになります。例えば、
...
といった文章を読み上げさせた時は、『「あっ、ヘリコプターだっ」』の部分を読み終わった時には『だれかがさけびました。』の最初の「だ」の位置が次の読み上げ開始位置として読み上げ開始位置を更新できるわけです。
そのような動作になりますため、会話文が多い小説の場合はなんとなく読み上げ開始位置が更新できるけれど、走れメロスのように会話文の指定が無いような文章の場合には全然読み上げ開始位置が更新されないという問題を抱えることになるわけです。

ここまでが、「設定タブ」->「読み上げ中の位置表示更新をサボるようにする」をONにした時に起こる事になります。

次に、この「設定タブ」->「読み上げ中の位置表示更新をサボるようにする」をONにした時に発生する、走れメロスのように会話文の指定が無いような文章の場合には全然読み上げ開始位置が更新されないという問題に対抗する方法について説明します。

こちらの対策としては、句読点や空白、改行といった物があったら、そこで発話を一旦止めて改めて発話を開始する、という事をするようにします。例えば、「メロスは激怒した。必ず、かの邪智暴虐の王を除かなければならぬと決意した。メロスには政治がわからぬ。メロスは、村の牧人である。笛を吹き、羊と遊んで暮して来た。けれども邪悪に対しては、人一倍に敏感であった。」という文章の場合、「メロスは激怒した。」「必ず、」「かの邪智暴虐の王を除かなければならぬと決意した。」というような感じで区切って読み上げさせ、その区切り毎に読み上げ開始位置を更新し続けることができるようになります。

ただ、読み上げを区切るようにしてしまうと、発話がなめらかにはつながらず、一呼吸おいて次の発話が始まるような形になります。この時、区切る頻度を高くしてしまうと発話時のテンポが悪くなってしまいます。そのため、この区切る時に最低でもこれくらいの文字数は続けて発話してほしい、という文字数を指定できるようにしました。その「最低でもこれくらいの文字数は続けて発話してほしい、という文字数」に当たる物が、「設定タブ」->「読み上げ中の位置表示更新をサボるようにする」がONの場合に表示される「一息で読み上げる範囲が長くなる場合にテキトーな範囲で区切るようにするが、その時に区切られなくなる最小の文字数〜」設定になります。概ね50文字程度に設定すると、読み上げ開始位置の更新がそこそこの頻度で発生するけれど、発話時のテンポはそこまで悪くならない、という感じに使えそうな気がします。ただ、これは開発者側の個人的な感想になりますので、ご利用されるユーザの方はご自身で最適な値を設定してください。

以上が今回追加された設定の解説になります。

まとめますと、上記の新しい2つの設定項目を適切に設定すれば、

・iOS 16.3 や iOS 16.3.1 でメモリリーク問題を回避できる
・「設定タブ」->「発話変更設定」で会話文の設定を削除したり動作しないように設定していたものを元に戻せる
・「設定タブ」->「読み上げ時の間の設定」の「読み上げ時の間の仕組み」を「非推奨型」から元に戻せる

という事ができるようになります。ただ、読み上げ時のテンポが少し悪くなってしまいます。
...
- Added "Setting tab" -> "Skip reading position display"
- When "Settings tab" -> "Skip the reading position display" is ON, added the setting "The minimum number of characters that will not be separated when dividing the reading range in a single breath when the reading range becomes long~"

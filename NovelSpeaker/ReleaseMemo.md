...
・読み上げが勝手に停止する問題を修正しました(多分これで根本的な対応になったはずです)

追伸
評価やレビュー、ありがとうございます。読み上げが勝手に停止する問題を直したものをリリース致します。
文章のコピペなどでの小説家になろう以外の小説の登録機能などは、まだまだ時間がかかってしまいそうです。首をながーーーーーーくして待っていただけると嬉しいです。
あと、毎回リリースのたびに少量ですが標準の読み替え辞書を更新しています。ユーザ様が登録した読み替え辞書と同じ元文字列のものがあると上書きされてしまいますが、[設定]→[標準の読みの修正を上書き追加]から上書き追加できますので、Siriさんが変な読み方をするなぁと思っている人は新しいバージョンがリリースされるたびに上書き追加していくと良いかもしれません。

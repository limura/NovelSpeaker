...
- 起動不能・クラッシュ・性能劣化などの障害では、いきなり実装修正を始めず、まず対象端末・OS・再現条件・クラッシュログ・メモリ/CPU/ディスク等を計測して記録する。
- 計測結果から原因候補を絞り、反証可能な診断を行ってから、影響範囲を定めて実装修正に進む。
- 特定の端末名だけを原因とみなさず、メモリ容量・OS・CPU・機能利用状況などの共通条件で比較する。
